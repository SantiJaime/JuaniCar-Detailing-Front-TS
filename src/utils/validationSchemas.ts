...
export const errorTurnSchema = yup.object().shape({
  ...
  details: yup.string(),
  vehicle: yup.string().required("Campo tipo de vehículo obligatorio"),
  service: yup.string().required("Campo servicio a solicitar obligatorio"),
  date: yup.string().required("Campo fecha obligatorio"),
  hour: yup.string().required("Campo horario obligatorio"),
});
